  ...
  def createDeviceSummary(silverDf: DataFrame, outputPath: String): Unit = {
    ...
    val deviceSummary = silverDf
      ...
      .withColumn("device_health", 
        when(col("device_quality_score") >= 0.9 && col("active_hours") >= 20, "excellent")
        .when(col("device_quality_score") >= 0.8 && col("active_hours") >= 16, "good")
        .when(col("device_quality_score") >= 0.7 && col("active_hours") >= 12, "acceptable")
